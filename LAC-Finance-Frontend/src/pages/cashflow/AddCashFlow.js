...
export default function AddCashFlow(props) {
    ...
    const action = props.cashFlowData && props.cashFlowData.RecordId > 0 ? 'Update Cash Flow' : 'Add Cash Flow'
    ...
    const showInputErrors = () => {
        ...
        if (!cashFlowDetails.InvestmentCost && !cashFlowDetails.InvEstimatedValue) {
            inputErrors.estimatedValue = 'Please enter either investment cost or estimated value';
        }
    ...
    const handleChange = (event) => {
        ...
        switch (name) {
            case 'PortCoId':
                inputErrors.portCoId = value.length <= 0 ? 'Please select portco name' : '';
                break;
            case 'FundId':
                inputErrors.fundId = value.length <= 0 ? 'Please select fund type' : '';
                break;
            case 'ShareClassId':
                inputErrors.shareClassId = value.length <= 0 ? 'Please select share class' : '';
                break;
    ...
    const validateAllInputs = () => {
        if (!cashFlowDetails.PortCoId || !cashFlowDetails.ShareClassId || !cashFlowDetails.FundId || !cashFlowDetails.Date ||
                ...
                <DialogContent>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            ...
                            {errors.portCoId.length > 0 && <span className='error'>{errors.portCoId}</span>}
                        </Grid>
                        <Grid item xs={12}>
                            <FormControl fullWidth variant="outlined" required size="small">
                            ...
                            {errors.fundId.length > 0 && <span className='error'>{errors.fundId}</span>}
                        </Grid>
                        <Grid item xs={12}>
                            <FormControl fullWidth variant="outlined" required size="small">
                            ...
                            {errors.shareClassId.length > 0 && <span className='error'>{errors.shareClassId}</span>}
                        </Grid>
                        <Grid item xs={12}>
                        ...
                        <Grid item xs={12}>
                            <TextField fullWidth name="InvestmentCost" label="Investment Cost" size="small"
                        ...
                        <Grid item xs={12}>
                            <TextField fullWidth name="InvEstimatedValue" label="Estimated Value" size="small"
                                onChange={handleChange} noValidate value={cashFlowDetails.InvEstimatedValue}
                                variant="outlined" style={{ fontFamily: 'poppins' }}
                                InputProps={{
                                    endAdornment: <InputAdornment position="end">$</InputAdornment>,
                                }} />
                            {errors.estimatedValue.length > 0 && <span className='error'>{errors.estimatedValue}</span>}
                        </Grid>
                    </Grid>
                </DialogContent>
                <DialogActions style={{ padding: '8px 24px 16px 24px' }}>
                    <Button fullWidth onClick={addCashFlow} color="primary" className={classes.customButtonPrimary} size="medium">
                        {action}
                    </Button>
                </DialogActions>
            </Dialog>
